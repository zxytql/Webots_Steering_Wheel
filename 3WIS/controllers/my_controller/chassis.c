...
void Helm_Evaluation(helm_chassis_t *chassis)
{
	...
	float s1 = sinf(chassis->wheel1.rad_actual) / DIS_WHEEL2CENTER;
	float s2 = -sinf(chassis->wheel2.rad_actual + M_PI / 3) / DIS_WHEEL2CENTER;
	float s3 = cosf(chassis->wheel3.rad_actual + M_PI / 6) / DIS_WHEEL2CENTER; //好像没乘距离？
	...
	cal_vx = (chassis->wheel1.v_target * cosf(chassis->wheel1.rad_actual) + 
				...
				chassis->wheel3.v_target * cosf(chassis->wheel3.rad_actual)) / 3;
	
	cal_vy = (chassis->wheel1.v_target * sinf(chassis->wheel1.rad_actual) + 
				chassis->wheel2.v_target * sinf(chassis->wheel2.rad_actual) + 
				chassis->wheel3.v_target * sinf(chassis->wheel3.rad_actual)) / 3;

	cal_wz = (s1 * chassis->wheel1.v_target + s2 * chassis->wheel2.v_target + s3 * chassis->wheel3.v_target) / 3;
	
	printf("s1 = %f, s2 = %f, s3 = %f \n", s1, s2, s3);
	printf("v1 = %f, v2 = %f, v3 = %f \n", chassis->wheel1.v_target, chassis->wheel2.v_target, chassis->wheel3.v_target);
